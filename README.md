...
| :--- | :--- | :--- | :--- |
| **Nguyễn Tuấn Việt** | ** Data Engineer** | Xử lý dữ liệu đầu vào, pipeline tiền xử lý và hỗ trợ giám sát hệ thống |Thu thập, mở rộng và làm sạch bộ dữ liệu để đảm bảo chất lượng đầu vào cho mô hình.Xây dựng pipeline tiền xử lý dữ liệu xuất dữ liệu sạch phục vụ huấn luyện.Tạo và bổ sung các trường hợp dữ liệu khó để giảm tỷ lệ dự đoán nhầm.Phối hợp kiểm thử dữ liệu đầu vào thực tế, phát hiện các trường hợp mô hình dễ sai để cải thiện tập train.Phát triển module logging và giám sát kết quả dự đoán của hệ thống:ghi nhận comment người dùng gửi vào,lưu kết quả phân loại toxic / safe,theo dõi độ tin cậy của mô hình,hỗ trợ debug lỗi và thống kê hiệu năng hệ thống khi chạy thực tế. |
